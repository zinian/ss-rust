...
if [ $V2_VER != $v2_ver ];then
  ...
  wget https://github.com/shadowsocks/v2ray-plugin/releases/download/$V2_VER/v2ray-plugin-linux-amd64-$V2_VER.tar.gz &&\
  tar -xvf  v2ray-plugin-linux-amd64-$V2_VER.tar.gz -C /usr/local/bin &&\
  rm -rf v2ray-plugin-linux-amd64-$V2_VER.tar.gz &&\
  mv -f /usr/local/bin/v2ray-plugin_linux_amd64 /usr/local/bin/v2ray-plugin  &&\
  restart_rc="1"
fi
